...
GpuCollisionNarrowPhase::GpuCollisionNarrowPhase()
    ...
    , d_bigCellProfileInternals(initData(&d_bigCellProfileInternals, false, "bigCellProfileInternals", "Diagnostic-only clock64 phase and rejection-counter instrumentation inside the fused big-cell kernel. Perturbs performance and disables CUDA-graph replay; keep false outside profiling runs."))
        ...
        if (m_backendAvailable && d_useDenseGrid.getValue())
        {
            ...
            else if (usingIndexedDenseGrid && d_useFeatureBasedProximity.getValue())
            {
                ...
                if (d_useBigCellFusedGeneration.getValue())
                {
                    ...
                    bigConfig.profileFusedInternals = d_bigCellProfileInternals.getValue();
