...
# Runs testscenes/collisiondetectiontests/self_collision_vertex_triangle.py with FBP + v-t
...
LOG_DIR="${SOFA_BENCHMARK_LOG_DIR:-${REPO_DIR}/output/benchmark_logs/vt_smoke_self_collision_$(date +%Y%m%d_%H%M%S)}"
...
"${SOFA_ROOT}/bin/runSofa" \
    ...
    "${REPO_DIR}/testscenes/collisiondetectiontests/self_collision_vertex_triangle.py"
