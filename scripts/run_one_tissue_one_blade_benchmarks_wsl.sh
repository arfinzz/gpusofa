...
LOG_DIR="${SOFA_BENCHMARK_LOG_DIR:-${REPO_DIR}/output/benchmark_logs/one_tissue_one_blade_$(date +%Y%m%d_%H%M%S)}"
STEPS="${SOFA_BENCHMARK_STEPS:-60}"
if [[ -z "${SOFA_BENCHMARK_LABEL_SUFFIX:-}" ]]; then
    export SOFA_BENCHMARK_LABEL_SUFFIX="_$(date +%Y%m%d_%H%M%S)"
fi
...
echo "Running CPU one tissue / one blade benchmark..."
SOFA_BENCHMARK_LOG_DIR="${LOG_DIR}" run_sofa -g batch -n "${STEPS}" "${REPO_DIR}/test_cpu_one_tissue_one_blade_benchmark.py"

echo "Running GPU dense-grid one tissue / one blade benchmark..."
SOFA_BENCHMARK_LOG_DIR="${LOG_DIR}" run_sofa -g batch -n "${STEPS}" "${REPO_DIR}/test_gpu_one_tissue_one_blade_dense_grid_benchmark.py"

echo "CPU summary label: cpu_one_tissue_one_blade_benchmark${SOFA_BENCHMARK_LABEL_SUFFIX}"
echo "GPU summary label: gpu_one_tissue_one_blade_dense_grid_benchmark${SOFA_BENCHMARK_LABEL_SUFFIX}"
