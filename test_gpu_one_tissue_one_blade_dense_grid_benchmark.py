...
USE_VERTEX_TRIANGLE_PROXIMITY = env_flag("SOFA_USE_VERTEX_TRIANGLE_PROXIMITY", False)
PROXIMITY_COMPUTE_BARYCENTRICS = env_flag("SOFA_PROXIMITY_COMPUTE_BARYCENTRICS", True)
PROXIMITY_READ_CONTACT_COUNTER = env_flag("SOFA_PROXIMITY_READ_CONTACT_COUNTER", False)
...
def createScene(root):
    ...
    root.addObject(
        ...
        label='gpu_one_tissue_one_blade_dense_grid_benchmark' + BENCHMARK_LABEL_SUFFIX,
        outputDir=BENCHMARK_LOG_DIR,
        pipelinePhase='plan-b-dense-grid-detection-only',
        tissueSolver='none',
        tissueForceField='none',
        collisionStateTemplate='CudaVec3f',
        collisionMapping='none (direct GPU collision surfaces)',
        visualMapping='none',
        notes='One tissue surface and one blade mesh. Dense-grid GPU narrow phase uses direct CUDA positions, indexed topology, GPU hash dedupe, optional experimental batched insertion/active-cell compaction, and keeps counters/contacts device-side when collision response is disabled.',
        simVertexCount=0,
        simElementCount=0,
