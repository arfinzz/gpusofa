...
in shared memory and running the FBP closest-feature math inline — no candidate-pair list,
no separate FBP launch) — joins the five below. Contact-identical everywhere;
**fastest of all eight measured configurations on the 14,368 scene (0.309 ms narrow
kernel)** and ties the sorted grid at the 200k bench. The two build strategies are a
measured A/B: CSR beats a literal per-big-cell open-addressing hash build ~2.9×
(`bigCellUseHashBuild`). Details: `reports/performance_all_modes_20260715.md`.
Previously (2026-07-03): a fifth broad-cull "way" — the **sorted-grid / tiled
...
ordering): `reports/performance_all_modes_20260715.md`, plan §5.23.
...
`reports/performance_all_modes_20260715.md`** (earlier dated reports are under
...
path — **six ways** (all bit-identical contacts, see
...
| 5. sorted grid (tiled binning) | `useSortedGridGeneration=1` | large + dense pairs (no per-cell caps) |
| 6. big-cell fused gen+narrow | `useBigCellFusedGeneration=1` | **fastest everywhere** with the default shared-hash build (0.290 ms on 14k; 0.56/0.81 ms at 80k/200k — 16–26% ahead of way 5) |

The exact-contact and v-t paths always use the dense grid; precedence when
several flags are set: bigcell > hash > simple > sorted > dense. Way 6 is the
only way with **no candidate-pair list and no separate FBP launch** — the
narrow-phase math runs inline in the generation kernel against shared-memory
staged tool vertices.
...
| `useBigCellFusedGeneration` | **false** | **EXPERIMENTAL "6th way", DEFAULT OFF (2026-07-12).** Big-cell / small-cell **FUSED** generation + narrow phase: small cells grouped into `bigCellFactor`³ big cells; per-big-cell CSR table of packed (triangle, local small cell) entries (count → scan → fill; the scan input is factor³ smaller than way 5's); then **one kernel per mixed big cell** stages the tool side (ids + AABBs + all three **vertices**, ≈18 KB shared) via an in-shared 64-bin counting sort and sweeps the tissue entries — AABB pre-cull + way-5's home-cell exactly-once rule at small-cell granularity (**pair set provably identical to way 5's**) + the identical FBP math inline. No candidate-pair list, no separate FBP launch. **Measured: fastest of all 8 configs on the 14,368 scene (0.309 ms vs sorted 0.337 / simple 0.332 / hash 0.339); ties way 5 at the 200k bench (1.09 ms).** Precedence: bigcell first. Requires `useFeatureBasedProximity=true`. See `reports/performance_all_modes_20260715.md` |
| `bigCellFactor` | **2** | Big-cell edge in small cells (power of two ≤ 4). **Measured default 2**: factor 4 collapses block-level parallelism (48 mixed big cells at 200k → 1.52 ms vs 1.09 at factor 2) |
| `bigCellToolTile` | **256** | Tool entries staged in shared per chunk (1..256); oversized big cells loop over chunks (correct at any size; tile 32 costs ~17%) |
| `bigCellUseHashBuild` | **false** | Build-strategy A/B: false = CSR (count → scan → fill; big-cell id is a perfect direct index); true = **literal per-big-cell open-addressing hash multi-map** (atomicCAS probing into fixed slot regions, cleared each frame). **Measured ~2.9× slower than CSR** (0.71 → 2.02 ms full pipeline at 80k) — kept as the honest answer to "wouldn't a hash table be faster?" |
| `bigCellHashSlots` | **1024** | Hash-build only: slots per (big cell, side) region (pow2, 64–4096). 1024 dropped 256 entries on the 80k bench (best-effort, reported via `buildOverflowCount`); **2048 = zero-overflow** there |
| `bigCellSharedBuild` | **1** | "Populate in shared, merge later" A/B (2026-07-15): each block takes a fixed triangle chunk and builds its histogram/CSR contribution in shared memory, merging with per-bin instead of per-entry global atomics. 0 = off; **1 = shared hash table (DEFAULT — −12%/−26% full pipeline at 80k/200k; way 6 now beats way 5 by 22–26% on the bench and holds the overall record 0.290 ms on the 14k scene)**; 2 = shared sorted list via bitonic sort (measured ~2.6× slower — the in-shared sort costs more than the atomics it avoids; the exact mirror of the global-build A/B where sorting won). Contacts identical; spills fall back to the direct path (`sharedSpillCount`). See `reports/archive_pre_20260715/bigcell_shared_build_ab_20260715.md` |
